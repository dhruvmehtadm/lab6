(function (window, undefined) {
  if (window.document && window.Worker) {
    var worker = new Worker("./tests/js-shapefile-to-geojson/shapefile.js");

    var Shapefile = function (o, callback) {
      var w = (this.worker = worker),
        t = this,
  ...
  Shapefile.prototype = {
    ...
    formatIntoGeoJson: function () {
      ...
      geojson.bbox = [bounds.left, bounds.bottom, bounds.right, bounds.top];
      ...
      for (var r = 0, record; (record = records[r]); r++) {
        ...
        switch (record.shapeType) {
          case "Point":
            geometry.type = "Point";
            geometry.coordinates = [record.points.x, record.points, y];
            break;
          case "MultiPoint":
          case "PolyLine":
            geometry.type =
              record.shapeType == "PolyLine" ? "LineString" : "MultiPoint";

  ...
  var geostats = function (a) {
    ...
    this.setPrecision = function (decimals) {
      ...
      if (this.precisionflag == "auto") {
        for (var i = 0; i < this.serie.length; i++) {
          ...
          if (
            !isNaN(this.serie[i] + "") &&
            (this.serie[i] + "").toString().indexOf(".") != -1
          ) {
      ...
      if (this.precision > 20) {
        ...
        this.log(
          "this.precision value (" +
            this.precision +
            ') is greater than max value. Automatic set-up to 20 to prevent "Uncaught RangeError: toFixed()" when calling decimalFormat() method.'
        );
        this.precision = 20;
      }

      this.log(
        "Calling setPrecision(). Mode : " +
          this.precisionflag +
          " - Decimals : " +
          this.precision
      );
    ...
    this.median = function () {
      ...
      if (this.stat_median == null) {
        ...
        if (tmp.length % 2) {
          ...
          this.stat_median =
            (parseFloat(tmp[tmp.length / 2 - 1]) +
              parseFloat(tmp[tmp.length / 2])) /
            2;
        }
      }
    ...
    this.variance = function () {
      ...
      if (this.stat_variance == null) {
        ...
        if (round == true) {
          this.stat_variance =
            Math.round(this.stat_variance * Math.pow(10, this.roundlength)) /
            Math.pow(10, this.roundlength);
        }
      }
    ...
    this.stddev = function (round) {
      ...
      if (this.stat_stddev == null) {
        ...
        if (round == true) {
          this.stat_stddev =
            Math.round(this.stat_stddev * Math.pow(10, this.roundlength)) /
            Math.pow(10, this.roundlength);
        }
      }
    ...
    this.cov = function (round) {
      ...
      if (this.stat_cov == null) {
        ...
        if (round == true) {
          this.stat_cov =
            Math.round(this.stat_cov * Math.pow(10, this.roundlength)) /
            Math.pow(10, this.roundlength);
        }
      }
    ...
    this._nodata = function () {
      if (this.serie.length == 0) {
        if (this.silent)
          this.log(
            "[silent mode] Error. You should first enter a serie!",
            true
          );
      ...
      if (isNaN(nbclassTmp)) {
        if (this.silent)
          this.log(
            "[silent mode] '" +
              nbclassTmp +
              "' is not a valid integer. Enable to set class number.",
            true
          );
        else
          throw new TypeError(
      ...
      if (array[0] !== this.min() || array[array.length - 1] !== this.max()) {
        if (this.silent)
          this.log(
            "[silent mode] " +
              t(
                "Given bounds may not be correct! please check your input.\nMin value : " +
                  this.min() +
                  " / Max value : " +
                  this.max()
              ),
    ...
    this.getClassEqInterval = function (nbClass, forceMin, forceMax) {
      ...
      // we specify the classification method
      this.method =
        _t("eq. intervals") + " (" + nbClass + " " + _t("classes") + ")";
    ...
    this.getClassStdDeviation = function (nbClass, matchBounds) {
      ...
      a[nbClass] =
        typeof matchBounds === "undefined"
          ? a[nbClass - 1] + tmpStdDev
          : tmpMax;
      ...
      // we specify the classification method
      this.method =
        _t("std deviation") + " (" + nbClass + " " + _t("classes") + ")";
      ...
      if (this._hasNegativeValue() || this._hasZeroValue()) {
        if (this.silent)
          this.log(
            "[silent mode] " +
              _t(
                "geometric progression can't be applied with a serie containing negative or zero values."
              ),
            true
          );
        else
    ...
    this.getInnerRanges = function () {
      ...
      for (var i = 0; i < tmp.length; i++) {
        ...
        if (cnt == this.bounds.length - 1) {
          ...
          a[cnt - 1] =
            "" + range_firstvalue + this.separator + tmp[tmp.length - 1];
    ...
    this.getHtmlLegend = function (
        ...
        if (this.counter.indexOf(0) !== -1) {
          if (this.silent)
            this.log(
              "[silent mode] " +
                _t(
                  "Geostats cannot apply 'discontinuous' mode to the getHtmlLegend() method because some classes are not populated.\nPlease switch to 'default' or 'distinct' modes. Exit!"
                ),
              true
            );
          else
      ...
      if (ccolors.length < this.ranges.length) {
        if (this.silent)
          this.log(
            "[silent mode] " +
              _t("The number of colors should fit the number of ranges. Exit!"),
            true
          );
        else
          throw new TypeError(
      ...
      if (this.is_uniqueValues == false) {
        for (var i = 0; i < this.ranges.length; i++) {
          if (getcounter === true) {
            cnt =
              ' <span class="geostats-legend-counter">(' +
              this.counter[i] +
              ")</span>";
          }
          ...
          if (mode == "distinct" && i != 0) {
            if (isInt(start_value)) {
              ...
              if (this.precisionflag == "manual" && this.precision != 0)
                start_value = parseFloat(start_value).toFixed(this.precision);
            } else {
              start_value =
                parseFloat(start_value) + 1 / Math.pow(10, this.precision);
          ...
          var block =
            '<div><div class="geostats-legend-block" style="background-color:' +
            ccolors[i] +
            '"></div> ' +
            el +
            cnt +
      ...
      var content =
        '<div class="geostats-legend"><div class="geostats-legend-title">' +
        _t(lg) +
        "</div>";
